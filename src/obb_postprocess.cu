...
namespace obb {
    ...
    static __device__ float box_probiou(
        ...
        float t3 = logf(
            ((a1 + a2) * (b1 + b2) - powf(c1 + c2, 2)) / (4 * sqrtf(fmaxf(a1 * b1 - c1 * c1, 0.0f)) * sqrtf(
                                                              fmaxf(a2 * b2 - c2 * c2, 0.0f)) + eps) + eps);
    ...
    void decode_kernel_invoker(float *predict, int num_bboxes, int num_classes, int output_cdim,
        ...
        checkKernel(decode_kernel<<<grid, block, 0,
                    stream>>>(predict, num_bboxes, num_classes, output_cdim, confidence_threshold, invert_affine_matrix,
                        parray,
                        MAX_IMAGE_BOXES));

        grid = grid_dims(MAX_IMAGE_BOXES);
        block = block_dims(MAX_IMAGE_BOXES);
        checkKernel(nms_kernel<<<grid, block, 0, stream>>>(parray, MAX_IMAGE_BOXES, nms_threshold));
    }
};
