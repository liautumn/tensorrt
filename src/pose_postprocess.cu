#include "pose_postprocess.cuh"
#include "logger.h"

namespace pose {
    // left, top, right, bottom, confidence, keepflag, (x, y, conf) * 17
    static __device__ void affine_project(float *matrix, float x, float y, float *ox, float *oy) {
        *ox = matrix[0] * x + matrix[1] * y + matrix[2];
        *oy = matrix[3] * x + matrix[4] * y + matrix[5];
    }

    static __global__ void decode_kernel_v8_Pose(float *predict, int num_bboxes, float confidence_threshold,
                                                 int output_cdim,
    ...
    void decode_kernel_invoker(float *predict, int num_bboxes, int output_cdim,
        ...
        checkKernel(decode_kernel_v8_Pose<<<grid, block, 0,
                    stream>>>(predict, num_bboxes, confidence_threshold, output_cdim, invert_affine_matrix, parray,
                        MAX_IMAGE_BOXES));

        grid = grid_dims(MAX_IMAGE_BOXES);
        block = block_dims(MAX_IMAGE_BOXES);
        checkKernel(nms_kernel_v8_Pose<<<grid, block, 0, stream>>>(parray, MAX_IMAGE_BOXES, nms_threshold));
    }
}
