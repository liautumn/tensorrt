...
//
// extern "C" {
// __declspec(dllexport) bool TensorRT_INIT_SYNC(const char *engine_file, float confidence, float nms) {
...
// }
//
// __declspec(dllexport) yolo::Box *TensorRT_INFER_SYNC(const unsigned char *image, const int *width, const int *height,
//                                                      int *size) {
//     trt::Timer timer;
//     auto img = yolo::Image(image, *width, *height);
//     timer.start();
//     auto boxes = my_yolo->forward(img);
//     timer.stop("batch 1");
...
// }
//
// __declspec(dllexport) void FreeMemory_SYNC(yolo::Box *ptr) {
//     delete[] ptr;
// }
// }
